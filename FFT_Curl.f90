! Program to calculate the Curl of a 3 dimensional vector function by serially computing the FFT
...
program fft_curl
    ...
    call cpu_time(T3)
    print *, "Allocate and define var", T3-T1
    ...
    ! Need layers of derivatives
    ! Find each partial derivative along those directions
    ! For direction i, find derivatives of functions j, j!=i
    ! Store these partial derivatives in a new matrix and then use this to find curl
    ...
    do i=1,n
        do j=1,n
            ...
            derz(i,j,:,1) = der1z/(nz*1.0)

            call dfftw_execute_dft_r2c(forward1z, func(i,j,:,2), fhat1z)
            dfhat1z = kappaz*Z*fhat1z
            call dfftw_execute_dft_c2r(backward1z, dfhat1z, der1z)
            derz(i,j,:,2) = der1z/(nz*1.0)
        enddo
    enddo

    curl(:,:,:,1) = dery(:,:,:,3) - derz(:,:,:,2)
    curl(:,:,:,2) = derz(:,:,:,1) - derx(:,:,:,3)
    curl(:,:,:,3) = derx(:,:,:,2) - dery(:,:,:,1)
    ...
    call cpu_time(T2)
    print *, "Calculating curl and destroy plan", T2-T5
