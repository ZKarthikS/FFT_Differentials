! Program to find the Gradient of a 3 dimensional scalar function by parallelly computing the FFT
...
program fft_para_grad
    ...
    real(8), allocatable :: actual_grad(:,:,:,:), grad(:,:,:,:), func(:,:,:), gradtemp(:,:,:,:)
    ...
    if( .not. allocated(gradtemp) ) allocate(gradtemp(nx,ny,nz,3))
    ...
    if(rank /= 0) then
        ...
        call MPI_SEND(grad, n*n*n*3, MPI_REAL8, 0, rank, MPI_COMM_WORLD, ierr)
    end if
    ...
    if(rank==0) then
        do i=1,nprocs-1
            ...
            call MPI_RECV(gradtemp, n*n*n*3, MPI_REAL8, i, i, MPI_COMM_WORLD, status1, ierr)
            ! Based on the received values from each of the processors, we can 
            ! rearrange them back into the master processor
            do l=1,n
                do j=istart2,iend2
                    grad(:,l,j,1) = gradtemp(:,l,j,1)
                    grad(l,:,j,2) = gradtemp(l,:,j,2)
                    grad(l,j,:,3) = gradtemp(l,j,:,3)
                enddo
            enddo
        enddo
    ...
    if(allocated(gradtemp)) deallocate(gradtemp)
